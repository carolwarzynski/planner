...
function Planner() {
  ...
  const adicionarCompromisso = () => {
    const novo = { id: Date.now(), titulo: `Compromisso ${compromissos.length + 1}`, concluido: false };
  ...
  const marcarComoConcluido = (id) => {
    setCompromissos(
      compromissos.map((c) => (c.id === id ? { ...c, concluido: !c.concluido } : c))
    );
  };
  ...
  // Drag & Drop
  const iniciarArrasto = (e, id) => e.dataTransfer.setData("text/plain", id);
      ...
      <div className="grade-container">
        ...
        {diasDaSemana.map((dia) => (
          <div key={dia} className="dia-coluna">
            ...
            {horarios.map((hora) => {
              const c = compromissos.find((c) => grade[`${dia}-${hora}`] === c.id);
              return (
                <div
                  key={hora}
                ...
                >
                  {c && (
                    ...
                    >
                      {c.titulo}
                      <div>
                        <button onClick={() => marcarComoConcluido(c.id)}>✅</button>
                        <button onClick={() => excluirCompromisso(c.id)}>🗑</button>
                      </div>
                    </div>
                  )}
                </div>
        ...
        <div className="lista-compromissos" onDrop={soltarNaLista} onDragOver={permitirSoltar}>
          <h2>Compromissos</h2>
